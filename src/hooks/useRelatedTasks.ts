...
export function useRelatedTasks(task: Task | null): RelatedTasksResult | undefined {
    return useLiveQuery(
        async () => {
            ...
            const parentId = splitFromEvent?.details?.sourceId;
            const parentTasks = parentId ? [await db.tasks.get(parentId)].filter(Boolean) as Task[] : [];

            // 分割先タスク（このタスクから分割された場合）
            const statusChangeEvent = history.find(h => h.type === 'STATUS_CHANGE' && h.details?.splitInto);
            const childIds = statusChangeEvent?.details?.splitInto || [];
            const childTasks = childIds.length > 0
                ? await db.tasks.where('id').anyOf(childIds).toArray()
                : [];

            // 統合元タスク（このタスクが統合先の場合）
            const mergedIntoEvent = history.find(h => h.type === 'MERGED_INTO' && h.details?.mergedFrom);
            const mergedFromIds = mergedIntoEvent?.details?.mergedFrom || [];
            const mergedFromTasks = mergedFromIds.length > 0
                ? await db.tasks.where('id').anyOf(mergedFromIds).toArray()
                : [];

            // 統合先タスク（このタスクが統合元の場合）
            const mergedToEvent = history.find(h => h.type === 'MERGED_INTO' && h.details?.targetId);
            const mergedIntoId = mergedToEvent?.details?.targetId;
